...
namespace Toolkid.UIGrid {
    [CustomPropertyDrawer(typeof(SeedData))]
    public class SeedDataDrawer : PropertyDrawer {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            if (property.isExpanded = EditorGUI.Foldout(arect, property.isExpanded, label)) {
                ...
                arect.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing * 2f;
                EditorGUI.PropertyField(arect, property.FindPropertyRelative("ripenTime"), new GUIContent("Ripen Time"));
                arect.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing * 2f;
